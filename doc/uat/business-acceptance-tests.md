...
| 3 | 60s 窗口内连发 3 次：<br>`for i in 1 2 3; do curl -sS -i -H "Authorization: Bearer $TOKEN" -H 'Content-Type: application/json' -X POST "$BASE/v1/audio/music" -d '{"prompt":"test"}'; done` | 第 3 次 **`429`**，`body.error.message` 为英文句子，形如 `token requests limit exceeded: 2 per minute (business_rate_limit_exceeded); retry after N s`（2026-09-09 起网关自身发起的这类 429 一律是英文——不要按旧的中文"令牌每分钟请求数已达上限"文本匹配，判定读 `error.code`），并带 `Retry-After` / `X-RateLimit-*` 头。<br>**判别点**：这是**业务限流**的 429，形状上区别于 Distribute 阶段"无可用渠道"的 404/503（前两次若因 UAT 无对应上游渠道返回 404/503，恰好证明请求确实到达了这个受保护的路由组）。若三次全是 404/503 而永远打不出 429，说明限流没有挂在 Distribute 之前 = FAIL |
