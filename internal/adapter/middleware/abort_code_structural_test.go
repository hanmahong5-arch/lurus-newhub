...
//
// The >30-sites / >5-files floor is a scanner-honesty guard: if a refactor
...
// vacuously passing on an empty set. At the time this test was written there
// were 39 call sites across 6 files (utils.go only defines the helper, it
// never calls itself, so it does not count toward filesTouched).
...
func TestAbortWithOpenAiMessage_EveryCallSiteCarriesACode(t *testing.T) {
	...
	if sitesScanned <= 30 {
		t.Fatalf("sitesScanned = %d, want > 30 (scanner-honesty floor; today's baseline is 39)", sitesScanned)
	}
